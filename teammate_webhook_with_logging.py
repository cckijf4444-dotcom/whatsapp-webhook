...
# MySQL 資料庫連線設定 (請根據您的本機或雲端資料庫資訊調整)
DB_HOST = os.environ.get("DB_HOST", "localhost")
DB_USER = os.environ.get("DB_USER", "root")
DB_PASSWORD = os.environ.get("DB_PASSWORD", "")
DB_NAME = os.environ.get("DB_NAME", "plant_info") # 對應您的資料庫名稱

def query_plant_from_db(yolo_label: str):
    """直接在 Python 中連線 MySQL 查詢植物資訊，取代原本的 PHP API"""
    try:
        ...
        with connection.cursor() as cursor:
            # 假設您的資料表名稱叫 plants，欄位包含 label, plant_name, amis_name, efficacy
...
# ==========================================
# ⚙️ 背景任務：串接 YOLO 伺服器、內部 MySQL 與 TTS
# ==========================================
def process_plant_image(image_id: str, sender_id: str):
    ...
    try:
        print(f"[{sender_id}] 步驟 1: 準備將圖片送往 YOLO 伺服器...")
        ...
        print(f"[{sender_id}] 步驟 2: YOLO 辨識成功 -> {yolo_label} (信心度: {confidence})")
        
        if confidence < 0.4:
            print(f"[{sender_id}] 信心度過低，略過後續流程。")
            return

        # 2. 直接查詢 MySQL 資料庫
        plant_data = query_plant_from_db(yolo_label)
        if not plant_data:
            print(f"[{sender_id}] 資料庫查無此標籤: {yolo_label}")
            return
        ...
        if tts_response.status_code == 200:
            audio_filename = f"reply_{image_id}.mp3"
            with open(audio_filename, "wb") as f:
                f.write(tts_response.content)
            print(f"[{sender_id}] 步驟 5: 語音生成完畢！")
            # 實際傳回 WhatsApp 的函數可在此解開
            # send_whatsapp_audio(sender_id, audio_filename)
        else:
            print(f"[{sender_id}] TTS 系統發生錯誤！")
...
async def whatsapp_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if message['type'] == 'image':
            ...
            print(f">>> 收到 WhatsApp 圖片請求，已派發至背景處理...")
